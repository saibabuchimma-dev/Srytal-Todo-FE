...
describe('TaskSearch', () => {
  it('calls onChange when typing', async () => {
    ...
    renderWithProviders(<TaskSearch value="" onChange={onChange} />, {
      withRouter: false,
    });
...
describe('TaskDetailsModal', () => {
  const task = {
    ...
    dueDate: '2026-02-01',
    createdAt: '',
    assignedTo: { _id: 'u1', fullName: 'Emma' },
  };
  it('returns null without a task', () => {
    renderWithProviders(
      <TaskDetailsModal opened onClose={jest.fn()} task={null} />,
      {
        withRouter: false,
      },
    );
  ...
  it('renders details for a task', () => {
    renderWithProviders(
      <TaskDetailsModal opened onClose={jest.fn()} task={task as never} />,
      {
        withRouter: false,
      },
    );
...
describe('EmployeeHeader', () => {
  it('shows a fallback with no selection and details when selected', () => {
    ...
    const { unmount } = renderWithProviders(<EmployeeHeader />, {
      withRouter: false,
    });
