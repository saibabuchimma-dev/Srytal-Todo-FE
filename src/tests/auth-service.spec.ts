...
describe('auth.service', () => {
  it('login normalizes the user and attaches the token', async () => {
    ...
    expect(user).toMatchObject({ id: 'u1', name: 'Sravani', fullName: 'Sravani', role: 'Admin', token: 'tok' });
  });

  it('login throws when token or user is missing', async () => {
    mockApi.post.mockResolvedValueOnce({ data: { data: { token: '', user: null } } });
