...
describe('employee.service', () => {
  ...
  it('getEmployeesPage maps the paginated envelope', async () => {
    ...
    expect(mockApi.get).toHaveBeenCalledWith('/employees/search', {
  ...
  it('createEmployee posts and normalizes', async () => {
    ...
    expect(mockApi.post).toHaveBeenCalledWith(
      '/employees',
      expect.objectContaining({ fullName: 'N' }),
    );
  ...
  it('updateEmployee puts and normalizes a raw response', async () => {
    ...
    expect(mockApi.put).toHaveBeenCalledWith('/employees/e7', {
      fullName: 'U',
    });
